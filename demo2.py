...
class ReportAgent(Agent):
    def generate(self, ticker: str, df_raw: pd.DataFrame, df_proc: pd.DataFrame,
        ...
        """Generate enhanced report with visualization and decision suggestion."""
        ...
        if not fc_future.empty:
            ...
            # log that we have no future forecast to plot
            self.log("No future forecast available to plot (fc_future empty).")

        # Short-term prediction marker
        if short_term_prob is not None and short_term_label is not None:
            last_date = hist["ds"].iloc[-1]
            last_price = hist["y"].iloc[-1]
            color = "green" if short_term_label == 1 else "red"
            symbol = "triangle-up" if short_term_label == 1 else "triangle-down"
            text = f"Prob={short_term_prob:.2f}, Action={action}"
            fig.add_trace(go.Scatter(
        ...
        if "Vol_20" in df_proc.columns:
            fig.add_trace(go.Scatter(x=df_proc["Date"], y=df_proc["Vol_20"], mode="lines", name="Volatility (20d)"),
                          row=2, col=1)
        ...
        # --- Save summary for report export ---
        summary = {
            "ticker": ticker,
...
if __name__ == "__main__":
    ...
    tickers = ["GOOG"]
    for t in tickers:
        orchestrator.run_for(ticker=t, period="10y", interval="1d", horizon=5, fc_periods=30, persist=True)
